...
class WjsslCssOutputFormat {
	...
	*/
	...
	public function __call($sMethodName, $aArguments) {
		...
		} else if(method_exists('WjsslCssOutputFormatter', $sMethodName)) {
	...
	public function getFormatter() {
		if($this->oFormatter === null) {
			$this->oFormatter = new WjsslCssOutputFormatter($this);
		}
	...
	public static function create() {
		return new WjsslCssOutputFormat();
	}
...
class WjsslCssOutputFormatter {
	private $oFormat;
	
	public function __construct(WjsslCssOutputFormat $oFormat) {
	...
	*/
	public function safely($cCode, $params) {
		if($this->oFormat->get('IgnoreExceptions')) {
			// If output exceptions are ignored, run the code with exception guards
			try {
				// return $cCode();
				return call_user_func_array($cCode, $params);
			} catch (WjsslCssOutputException $e) {
				return null;
			} //Do nothing
		} else {
			// Run the code as-is
	...
	*/
	public function implode($sSeparator, $aValues, $bIncreaseLevel = false) {
		...
		foreach($aValues as $mValue) {
			...
			if($mValue instanceof WjsslCssRenderable) {
