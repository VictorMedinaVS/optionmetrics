...
/**
 * WjsslCssRuleSet is a generic superclass denoting rules. The typical example for rule sets are declaration block.
 * However, unknown At-Rules (like @font-face) are also rule sets.
 */
abstract class WjsslCssRuleSet implements WjsslCssRenderable, WjsslCssCommentable {
	...
	public function addRule(WjsslCssRule $oRule, WjsslCssRule $oSibling = null) {
		$sRule = $oRule->getRule();
	...
	public function getRules($mRule = null) {
		if ($mRule instanceof WjsslCssRule) {
			$mRule = $mRule->getRule();
		}
		$aResult = array();
	...
	/**
	* Remove a rule from this WjsslCssRuleSet. This accepts all the possible values that @link{getRules()} accepts. If given a Rule, it will only remove this particular rule (by identity). If given a name, it will remove all rules by that name. Note: this is different from pre-v.2.0 behaviour of PHP-CSS-Parser, where passing a Rule instance would remove all rules with the same name. To get the old behvaiour, use removeRule($oRule->getRule()).
 * @param (null|string|Rule) $mRule pattern to remove. If $mRule is null, all rules are removed. If the pattern ends in a dash, all rules starting with the pattern are removed as well as one matching the pattern with the dash excluded. Passing a Rule behaves matches by identity.
	*/
	...
	public function callback_safely_func($oRule, $oOutputFormat) {
	...
	public function render(WjsslCssOutputFormat $oOutputFormat = null) {
		...
		foreach ($this->aRules as $aRules) {
			foreach($aRules as $oRule) {
				$sRendered = $oOutputFormat->safely(array($this, 'callback_safely_func'), array($oRule, $oOutputFormat));
				if($sRendered === null) {
					continue;
				}
				if($bIsFirst) {
