...
/**
 * The root WjsslCSSList of a parsed file. Contains all top-level css contents, mostly declaration blocks, but also any @-rules encountered.
 */
class WjsslCssDocument extends WjsslCSSBlockList {
	/**
	 * WjsslCssDocument constructor.
	 * @param int $iLineNo
	 */
	...
	/**
	 * Gets all WjsslCssDeclarationBlock objects recursively.
	 */
	 ...
	 * @param (object|string) $mElement the WjsslCSSList or WjsslCssRuleSet to start the search from (defaults to the whole document). If a string is given, it is used as rule name filter (@see{RuleSet->getRules()}).
	...
	public function render(WjsslCssOutputFormat $oOutputFormat = null) {
		if($oOutputFormat === null) {
			$oOutputFormat = new WjsslCssOutputFormat();
		}
